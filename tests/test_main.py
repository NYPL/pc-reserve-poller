...
_ENVISIONWARE_DATA = [
    (10000000, 'barcode1', 100, datetime(2023, 1, 1, 1, 0, 0), 'branch1',
     'area1', 'staff_override1'),
    (20000000, '25555000000000', 200, datetime(2023, 2, 2, 2, 0, 0), 'branch2',
     'area2', 'staff_override2'),
    (30000000, 'barcode3', 300, datetime(2023, 3, 3, 3, 0, 0), 'branch3',
     'area3', 'staff_override3'),
    (40000000, 'barcode4', 400, datetime(2023, 4, 4, 4, 0, 0), 'branch4',
     'area4', 'staff_override4')
]
...
_RESULTS = [
    {'patron_id': 'obf_id1', 'ptype_code': 1,
     'patron_home_library_code': 'lib1', 'pcode3': 10, 'postal_code': 'zip1',
     'geoid': 'geoid1', 'key': 'obf_key1', 'minutes_used': 100,
     'transaction_et': '2023-01-01', 'branch': 'branch1', 'area': 'area1',
     'staff_override': 'staff_override1', 'patron_retrieval_status': 'found'},
    {'patron_id': 'obf_id2', 'ptype_code': None,
     'patron_home_library_code': None, 'pcode3': None, 'postal_code': None,
     'geoid': None, 'key': 'obf_key2', 'minutes_used': 200,
     'transaction_et': '2023-02-02', 'branch': 'branch2', 'area': 'area2',
     'staff_override': 'staff_override2',
     'patron_retrieval_status': 'guest_pass'},
    {'patron_id': 'obf_id3', 'ptype_code': None,
     'patron_home_library_code': None, 'pcode3': None, 'postal_code': None,
     'geoid': None, 'key': 'obf_key3', 'minutes_used': 300,
     'transaction_et': '2023-03-03', 'branch': 'branch3', 'area': 'area3',
     'staff_override': 'staff_override3',
     'patron_retrieval_status': 'missing'},
    {'patron_id': 'obf_id4', 'ptype_code': 4,
     'patron_home_library_code': 'lib4', 'pcode3': 40, 'postal_code': 'zip4',
     'geoid': None, 'key': 'obf_key4', 'minutes_used': 400,
     'transaction_et': '2023-04-04', 'branch': 'branch4', 'area': 'area4',
     'staff_override': 'staff_override4', 'patron_retrieval_status': 'found'}
]
...
class TestMain:
    ...
    def test_main_one_iteration(self, set_env_vars, mock_helpers, mocker):
        ...
        mock_sierra_client.execute_query.side_effect = [None, _SIERRA_DATA]
        ...
        mock_sierra_query.assert_called_once_with(
            "'barcode1','25555000000000','barcode3','barcode4'")
        mock_sierra_client.execute_query.assert_has_calls([
            mocker.call("SET statement_timeout='5min';"),
            mocker.call('SIERRA QUERY')])
    ...
    def test_main_multiple_iterations(self, set_env_vars, mock_helpers,
        ...
        mock_envisionware_client.execute_query.side_effect = [
            _TEST_ENVISIONWARE_DATA[:4], _TEST_ENVISIONWARE_DATA[4:]]
        ...
        mock_s3_client.set_cache.assert_has_calls([
            mocker.call(
                {'pcr_key': '4', 'pcr_date_time': '2023-04-04 04:00:00'}),
            mocker.call(
                {'pcr_key': '6', 'pcr_date_time': '2023-06-06 06:00:00'})])
        mock_s3_client.close.assert_called_once()
    ...
    def test_main_no_sierra_results(self, set_env_vars, mock_helpers, mocker):
        ...
        mock_obfuscate = mocker.patch(
            'main.obfuscate', return_value='obfuscated')
        mock_avro_encoder = self._set_up_mock('main.AvroEncoder', mocker)
        mock_redshift_client = self._set_up_mock('main.RedshiftClient', mocker)
        ...
        mock_sierra_client = self._set_up_mock('main.PostgreSQLClient', mocker)
        mock_sierra_client.execute_query.side_effect = [None, []]
    ...
    def test_main_no_redshift_results(self, set_env_vars, mock_helpers,
        ...
        _TEST_ENVISIONWARE_DATA = [
            ...
            for i in range(1, 5)]
        _TEST_SIERRA_DATA = [
            ('barcode{}'.format(i), i+1, i, 'lib{}'.format(i), i)
            for i in range(1, 5)]
        _RESULTS = [
    ...
    def test_main_sierra_query_retry(self, set_env_vars, mocker, caplog):
        ...
        mock_sierra_client.connect.assert_called_once()
        assert mock_sierra_client.execute_query.call_count == 4
        assert caplog.text == ''
        mock_sierra_client.close_connection.assert_called_once()
    ...
    def test_main_sierra_query_timeout(self, set_env_vars, mocker, caplog):
        ...
        mock_sierra_client = self._set_up_mock('main.PostgreSQLClient', mocker)
        mock_sierra_client.execute_query.side_effect = [None] + ([
            PostgreSQLClientError('test error')]*5)
        ...
        mock_sierra_client.connect.assert_called_once()
        assert mock_sierra_client.execute_query.call_count == 6
